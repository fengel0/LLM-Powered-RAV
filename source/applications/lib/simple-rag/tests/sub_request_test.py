...
from tests.simple_request_test import LlamaIndexVectorStoreSessionConfig
...
class TestQdrantSubRAGQueries(TestDBRAGQueries):
    __test__ = True
    ...
    def setup_method_sync(self, test_name: str):
        ...
        self._rag_cfg = LlamaIndexRAGConfig(
            base_url=llm.OPENAI_HOST, api_key=llm.OPENAI_HOST_KEY
        )
    ...
    async def setup_method_async(self, test_name: str):
        ...
        self.rag_llm = LlamaIndexSubRAG(
            chat_builder=LlamaIndexSubQuestionBuilder(config=cfg)
        )
