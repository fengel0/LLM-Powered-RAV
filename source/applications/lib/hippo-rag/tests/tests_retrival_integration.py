...
class TestHippoRAGIndexer(TestDBRAGQueries):
    __test__ = True
    ...
    def setup_method_sync(self, test_name: str):
        ...
        self.neo4j = Neo4jContainer(username=NEO4J_USER, password=NEO4J_PASS)
    ...
    async def setup_method_async(self, test_name: str):
        ...
        result = await self.vector_store.index(
            docs=["Ich mag Bratwurst"],
            metadata={"other": "lol", "count": 5},
        )
        if result.is_error():
            raise result.get_error()
        result = await self.vector_store.index(
            docs=["Ich mag Eiscreme"],
            metadata={"other": "test", "count": 3},
        )
        if result.is_error():
            raise result.get_error()

        self.rag_llm = HippoRAG(
            vector_store_entity=QdrantEmbeddingStore(cfg_ent, embedder=embedder),
            vector_store_fact=QdrantEmbeddingStore(cfg_link, embedder=embedder),
            vector_store_chunk=QdrantEmbeddingStore(cfg_chunk, embedder=embedder),
            llm=llm_client,
            config=HippoRAGConfig(
                retrieval_top_k=5,  # max docs to return
                linking_top_k=5,
                passage_node_weight=0.5,  # graph edge weighting factor
                qa_top_k=5,  # number of docs passed to QA
                damping=0.5,
                directional_ppr=False,
            ),
            graph=db,
            filter=DSPyFilter(llm_model=llm_client, config=DSPyFilterConfig()),
            state_store=PostgresDBStateStore(),
        )
