...
from domain_test.enviroment import test_containers, rerank, embedding
...
class TestHippoImplementation(AsyncTestBase):
    ...
    async def test_embedd_file_runs_with_provisioned_env(self):
        ...
        # ----- Phase 2: hippo/graph implementation -----
        self._provisioner.restore()
        hippo_values = self._compose_env_values_hippo()
        self._provisioner.values = hippo_values
        self._provisioner.apply()
    ...
    def _compose_env_values_hippo(self) -> dict[str, object]:
        ...
        vals[OPENAI_KEY] = "http://127.0.0.1::11434"
        vals[OPENAI_MODEL] = "model"

        # switch to graph/hippo
        ...
        vals[QDRANT_PORT] = self._q_http_port
        vals[QDRANT_GRPC_PORT] = self._q_grpc_port
        vals[QDRANT_PREFER_GRPC] = "True"  # as string in your original test
        ...
        vals[EMBEDDING_SIZE] = int(embedding.EMBEDDING_SIZE)
        vals[EMBEDDING_CONFIG] = BASE_EMBEDD_CONFIG
